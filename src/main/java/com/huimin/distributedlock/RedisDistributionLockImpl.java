...
public class RedisDistributionLockImpl implements DistributionLock {
	...
	private static final LogUtil LOG = LogUtil.logger(RedisDistributionLockImpl.class);

	@Autowired
	private StringRedisTemplate redisTemplate;

	*//**
	 * 加锁 取到锁加锁，取不到锁一直等待直到获得锁
	 * 
	 * @param lockKey
	 * @param threadName
	 * @return
	...
	public  long lock(String lockKey, String threadName) {
		...
		while (true) { // 循环获取锁
			...
			try {
				...
				//STimeUnit.MILLISECONDS.sleep(200);
	...
	*//**
	 ...
	 *//*
	@Override
	public synchronized  void unlock(String lockKey, String lockValue, String threadName) {
		...
		if (currt_lock_timeout_str != null && currt_lock_timeout_str.equals(lockValue)) {
